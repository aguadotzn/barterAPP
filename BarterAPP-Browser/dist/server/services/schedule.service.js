...
function create(scheduleParam) {
    ...
    db.schedule.insert(scheduleParam, function (err, doc) {
        if (err)
            deferred.reject(err.name + ': ' + err.message);
        deferred.resolve();
    });
...
function move(text, company, start, end) {
    ...
    db.schedule.update({ text: text, company: company }, { $set: set }, function (err, doc) {
        if (err)
            deferred.reject(err.name + ': ' + err.message);
        deferred.resolve();
    });
...
function _delete(text, company) {
    var deferred = Q.defer();
    db.schedule.remove({ text: text, company: company }, function (err) {
        if (err)
            deferred.reject(err.name + ': ' + err.message);
        deferred.resolve();
    });
